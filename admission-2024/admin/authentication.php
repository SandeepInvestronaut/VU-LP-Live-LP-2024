...
session_start();
  ...
  //For live database
  $connection = mysqli_connect("vi-website-db.cyofdhdrzbwm.ap-south-1.rds.amazonaws.com", "root", "Web-vu#2020","enq_vupune");
	//Check connection
...
<!-------------------------------table------------------------------------------>
  <div class="container" style="margin-top: 50px">
    ...
    <div class="row">
      <div class="col-md-12">
        <table class="table table-bordered table-hover" id="enquiries-list">
          ...
          <tbody>
            <?php
              $sql = $conn->query('SELECT * FROM `lead` WHERE `courseName` = 1302 OR `courseName` = 1303 ORDER BY id DESC');

             

              while($data = $sql->fetch_array()) {
                if($data['courseName'] == 1302){
                  $cname = "M.S. (Engineering at the Nanoscale)";
                }else if($data['courseName'] == 1303){
                  $cname = "M.S. (Additive Manufacturing and Design)";
                }else{
                  $cname = NULL;
                }
                  echo '
